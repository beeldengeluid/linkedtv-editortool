		...
		if jsonData:            
			...
			for k in jsonData['results']['bindings']:
				...
				if RDFType == '%sConcept' % self.LINKEDTV_ONTOLOGY_PF:
					concepts.append(Concept(label, None, start, end, OWLSameAs, mfURI, annotationURI, bodyURI, r, c))
					...
					nes.append(NamedEntity(label, None, super(AutogenDataLoader, self).getNEType(DCType, RDFType, OWLSameAs), 
	...
	def __loadAutogenEnrichmentsOfMediaResource(self, mediaResourceID):
		...
		if jsonData:
			for k in jsonData['results']['bindings']:
				...
				enrichments.append(Enrichment(entityLabel, None, start, end, None, None, bodyURI, 1, 1, deeplink, poster, source,
